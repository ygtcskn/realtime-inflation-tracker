...
Comparisons:
  ...
  2. UMIDAS LSTM (B) vs XGBoost
  3. UMIDAS LSTM (B) vs AR
  4. UMIDAS LSTM (B) vs RW
  5. UMIDAS LSTM (B) vs C model
  6. UMIDAS LSTM (B) vs D model
  7. XGBoost vs LASSO
  8. AR vs RW
  9. C vs D model
 10. C vs D model — per country
...
# Folder name → (label, file_prefix). file_prefix is the part before
# "_test_predictions.csv" inside the folder. Defaults to folder name when None.
MODELS = {
    "B_LSTM":              ("UMIDAS_LSTM", None),
    "B_LASSO":             ("LASSO",       None),
    "B_XGB":               ("XGBoost",     None),
    "0_AR":                ("AR",          "AR"),
    "0_RW":                ("RW",          None),
...
def load_model_predictions(folder_name, file_prefix=None):
...
def format_pooled_row(name1, name2, result, loss1, loss2):
    ...
    return (f"{name1:>15} vs {name2:<15}  n={n:>5}  "
            ...
            f"L1={loss1:.4f}  L2={loss2:.4f}  → {verdict}")
...
def main():
    ...
    for folder, (label, prefix) in MODELS.items():
        try:
            preds[label] = load_model_predictions(folder, prefix)
            df = preds[label]
