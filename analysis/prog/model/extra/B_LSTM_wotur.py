...
OUT_DIR = DATA_OUTPUT / "B_LSTM_wotur"
...
RANDOM_SEED = 42
MODEL_NAME = "B_LSTM_wotur"

COUNTRIES = ["BR", "CA", "FR", "DE", "IN", "ID", "IT", "JP", "MX", "RU", "ZA", "KR", "GB", "US"]
...
def save_results(test_df, train_fits, df_actual, model_name, out_dir, countries):
    ...
    if len(train_df) > 0:
        train_df.to_csv(out_dir / f"{model_name}_train_predictions.csv", index=False)
    ...
    add_metrics("turkey_analysis", "WITHOUT_TR", test_df)
    ...
    if len(train_df) > 0:
        ...
        for country in countries:
            add_metrics("train_country", country, train_df[train_df["country"] == country])
...
if __name__ == "__main__":
    ...
    for h in HORIZONS:
        ...
        for t_date in tqdm(test_dates, desc=f"LSTM_wotur h={h}", unit="date"):
            train_idx = [
            ...
            y_pred_train = y_scaler.inverse_transform(
                predict(model, X_seq_train_scaled, X_static_train_scaled).reshape(-1, 1)
            ).flatten()
            ...
            for j in range(len(train_idx)):
                train_fits[(train_meta[j][1], h)][train_meta[j][0]] = y_pred_train[j]
