...
INPUT_PATH = DATA_FINAL / "D_panel.csv"
OUT_DIR = DATA_OUTPUT / "D_weekspecific_LSTM"
...
RANDOM_SEED = 42
MODEL_NAME = "D_weekspecific_LSTM"
...
def get_week_features(all_seq_cols, week_position):
    selected = []

    for col in all_seq_cols:
        is_week = any(col.endswith(f"_w{w}") for w in range(1, 5))

        if is_week:
            for w in range(1, week_position + 1):
                if col.endswith(f"_w{w}"):
                    selected.append(col)
                    break
        else:
            selected.append(col)
...
if __name__ == "__main__":
    ...
    for col in all_seq_cols + ar_lag_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce")

    df[all_seq_cols] = df[all_seq_cols].fillna(0).astype(np.float32)
    ...
    df_actual = df[["date", "country", "infl_yoy"]].copy()

    for h in HORIZONS:
        for wp in tqdm(WEEK_POSITIONS, desc=f"D_LSTM h={h}", unit="week"):
            week_seq_cols = get_week_features(all_seq_cols, wp)
            full_seq = create_sequences(df, week_seq_cols, static_cols, h, SEQ_LEN)
            ...
            for t_date in tqdm(test_dates, desc=f"  wp={wp}", unit="date", leave=False):
                tr_idx = [
                ...
                for j in range(len(tr_idx)):
                    train_fits[(m_tr[j][1], wp, h)][m_tr[j][0]] = yp_tr[j]
